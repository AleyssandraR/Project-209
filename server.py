...
def handleShowList(client):
    ...
    for c in clients:
        counter+=1
        clientAddress=clients[c]['address'][0]
        connectedWith=clients[c]['connected_with']
        message=''
        if(connectedWith):
            message=f'{counter}, {c}, {clientAddress}, Connected with {connectedWith}, tiul,\n'
        
        else:
            message=f'{counter}, {c}, {clientAddress}, available, tiul,\n'
